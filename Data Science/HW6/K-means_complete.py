...
def k_means_clustering(data, centroids, k):
    ...
    while not centroid_current.equals(centroid_last):
        ...
        for idx, position in centroid_current.iterrows(): # the idx will not be used. We just separate the index with the actual data "position" in the dataset. You will use "cluster_count" as the column index in "clusters".
            ...
            clusters[cluster_count] = (data - position).pow(2).sum(1).pow(0.5)
            
            # your code ends
            cluster_count += 1
        print("cluter is: ", clusters)
        ...
        for row_idx in range(len(clusters)):
            ...
            idx = 0
            for i in clusters.iloc[row_idx].argsort().argsort():
                
                if i == 0:
                    clusterIDs.append(clusters.columns[idx])
                    break
                idx += 1
            # your code ends
        # assign points to clusters. The information is saved in the list and assigned to the dataset.
        print("clusterIDs is:", clusterIDs)     
        ...
        cluster_idx =0
        for idx, attributes in data.iterrows():
            x = attributes['x']
            y = attributes['y']
            cluster_idx = attributes['Cluster']
            points[cluster_idx].append([x, y])   
        
        # calculate the centroids
        for i in range(k):
            centroids.append( np.mean(points[i], axis = 0))
       
        # your code ends
        centroid_current = pd.DataFrame(data=centroids, columns = ['x', 'y'])
        
        print("The new centroids are ", centroid_current) 
